...
var _ = Describe(SIG("Migration Stall Detection", Serial, decorators.RequiresTwoSchedulableNodes, func() {
	...
	BeforeEach(func() {
		...
		migrationPolicy.Spec.ExperimentalMigrationOptions = &v1.ExperimentalMigrationOptions{
			...
			StallDetector: &v1.StallDetectorOptions{
				...
				SwitchoverTimeout:    kvpointer.P(uint64(30)),
	...
	Context("with post-copy enabled", func() {
		It("should detect stall and switch to post-copy", func() {
			...
			migrationPolicy.Spec.MaxDowntimeMs = kvpointer.P(uint64(1))
	...
	Context("with natural convergence after reducing dirty rate", func() {
		It("should detect stall then converge naturally after stress is stopped", func() {
			...
			migrationPolicy.Spec.MaxDowntimeMs = kvpointer.P(uint64(400))
			// ensures migration won't be aborted for being too far off
			migrationPolicy.Spec.ExperimentalMigrationOptions.StallDetector.PrecopyPossibleFactor = kvpointer.P(float64(1000))
			...
			Eventually(func() string {
				return getVirtLauncherSourceLogs(virtClient, vmi)
			}, stallDetectionTimeout, 5*time.Second).Should(MatchRegexp(maxDowntimeSetString(*migrationPolicy.Spec.MaxDowntimeMs, *migrationPolicy.Spec.ExperimentalMigrationOptions.StallDetector.PrecopyPossibleFactor)))
	...
	Context("with relaxation mechanism", func() {
		It("should relax best remaining bytes when workload ramps up after initial progress", func() {
			...
			migrationPolicy.Spec.AllowWorkloadDisruption = kvpointer.P(true)
			migrationPolicy.Spec.MaxDowntimeMs = kvpointer.P(uint64(1))
			migrationPolicy.Spec.ExperimentalMigrationOptions.StallDetector.PrecopyPossibleFactor = kvpointer.P(float64(10000))
			migrationPolicy.Spec.ExperimentalMigrationOptions.StallDetector.StallMargin = kvpointer.P(float64(0.00))
	...
	Context("with post-copy and workload disruption both disabled", func() {
		It("should detect stall and abort migration when convergence is impossible", func() {
			...
			migrationPolicy.Spec.AllowWorkloadDisruption = kvpointer.P(false)
			migrationPolicy.Spec.MaxDowntimeMs = kvpointer.P(uint64(1))
			migrationPolicy.Spec.ExperimentalMigrationOptions.StallDetector.PrecopyPossibleFactor = kvpointer.P(float64(1))
			// without this, migration won't abort instantly since it would think
			migrationPolicy.Spec.ExperimentalMigrationOptions.StallDetector.CompletionTimeoutFactor = kvpointer.P(float64(100000))
			...
			Eventually(func() string {
				...
				MatchRegexp(abortImpossibleString(
					*migrationPolicy.Spec.MaxDowntimeMs,
					*migrationPolicy.Spec.ExperimentalMigrationOptions.StallDetector.PrecopyPossibleFactor,
				)),
			)
	...
	Context("with completion timeout", func() {
		setCompletionTimeoutTestPolicy := func() {
			...
			migrationPolicy.Spec.ExperimentalMigrationOptions.StallDetector.StallProgressTimeout = kvpointer.P(uint64(600))
			// prevents test from aborting because we would exceed the completion timeout several times over
			migrationPolicy.Spec.ExperimentalMigrationOptions.StallDetector.CompletionTimeoutFactor = kvpointer.P(100.0)
		}
